    ...
    <div class="grid grid-cols-3">
        <form action="{{ route('students.update', $student->id) }}" method="POST"
            ...
            <div class="flex flex-col gap-2">
                ...
                <select name="status" id="status" class="px-3 py-2 border border-zinc-300 appearance-none bg-slate-50">
                    <option value="" disabled 
                        {{ old('status', $student->status) == '' ? 'selected' : '' }}>Select Student Status</option>
                    <option value="Active" 
                        {{ old('status', $student->status) == 'Active' ? 'selected' : '' }}>Active</option>
                    <option value="Inactive" 
                        {{ old('status', $student->status) == 'Inactive' ? 'selected' : '' }}>On Leave</option>
                    <option value="Graduated"
                        {{ old('status', $student->status) == 'Graduated' ? 'selected' : '' }}>Graduated</option>
                    <option value="Dropped Out"
                        {{ old('status', $student->status) == 'Dropped Out' ? 'selected' : '' }}>Dropped Out</option>
                </select>
                @error('status')
                    <div class="text-red-500">{{ $message }}</div>
                @enderror
            ...
            <div class="self-end flex gap-2">
                ...
                <button type="submit"
                        class="bg-blue-500 border border-blue-500 text-blue-500 px-3 py-2 flex items-center gap-2 cursor-pointer">
