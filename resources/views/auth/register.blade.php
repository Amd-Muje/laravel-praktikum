<x-auth-layout title="Register" section_title="Register" section_description="Register using your email">
    <form action="{{ route('auth.store') }}" method="POST" class="flex flex-col gap-4 mt-2">
        @csrf
        @method('POST')
        <div class="flex flex-com gap-2">
            <label for="name" class="font-semibold text-sm">
                Name
            </label>
            <input type="text" id="name" 
            name="name" class="px-3 py-2 border border-zinc-300 bg-slate-50"
        ...
        <div class="flex flex-com gap-2">
            ...
            name="email" class="px-3 py-2 border border-zinc-300 bg-slate-50"
        ...
        <div class="flex flex-com gap-2">
            ...
            name="password" class="px-3 py-2 border border-zinc-300 bg-slate-50"
        ...
        <div class="flex flex-com gap-2">
            <label for="confirm_password" class="font-semibold text-sm">
                Confirm Password
            </label>
            <input type="confirm_password" id="confirm_password" 
            name="confirm_password" class="px-3 py-2 border border-zinc-300 bg-slate-50"
        ...
        <p class="text-zinc-600 text-sm text-center">
            ...
            <a href="{{ route('auth.login') }}" class="text-blue-500 hover:text-blue-600">Login Here</a>
        </p>
    </form>
